...
if(ISFRAME && !parent.document.getElementById('leftmenu')) {
	...
	<div class="container" id="cpcontainer">
		...
		<tr class="hover">
			<td class="td25"><input class="checkbox" type="checkbox" name="tidarray[<?php echo $val['id']; ?>]" value="<?php echo $val['classid']; ?>" /></td>
			<td><a href="detail.php?id=<?php echo $val['id']; ?>" target="_blank"><?php echo $val['title']; ?></a></td>
			<td><a href="list.php?classid=<?php echo $val['classid']; ?>" target="_blank"><?php echo $val['classname']; ?></a></td>
			<td><?php echo $val['username']; ?></td>
			<td class="td25"><?php echo $val['replycount']; ?></td>
			<td class="td25"><?php echo $val['hits']; ?></td>
			<td><?php echo formatTime($val['addtime'],false); ?></td>
		</tr>
		<?php }}?>
		<tr><td class="td25" colspan="7"><input type="submit" class="btn" id="submit_modsubmit" name="modsubmit" title="按 Enter 键可随时提交你的修改" value="放入回收站" /></td></tr>
		</table>
		</form>
